            ...
            <div class="modal-body p-0">
                <table class="table table-sm">
                    ...
                    <tbody id="investigation-table">
                        <tr>
                            <td class="text-center" colspan="4"><div class="mr-2 spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div></td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="4">
                                ...
                                <h4 data-toggle="collapse" data-target="#add-investigations" class="clickable col-md-12 my-2 cursor-pointer btn btn-outline-info w-auto m-0 btn-outline">Add New Item</h4>
                                <form id="add-investigations" action="{{ route('admin.investigations.store') }}" method="POST" class="collapse form-inline row justify-content-between">
                                    @csrf
                                    <input id="investigations-user-id" type="hidden" name="user_id" value="{{ $user_id ?? null }}">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label">Investigation Title</label>
                                            <input name="title" placeholder="Title" type="text" class="form-control form-control-sm w-100" list="investigations-list" autocomplete="off" required>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input name="details" placeholder="Details" type="text" class="form-control form-control-sm w-100" required>
                                        </div>
                                    </div>
                                    <div class="col-md-12 mt-1">
                                        <div class="row">
                                            <div class="form-group col-6">
                                                ...
                                                <input name="data_date[]" placeholder="Date" type="date" class="form-control form-control-sm w-100">
                                            </div>
                                            <div class="form-group col-6">
                                                <label class="control-label">Report</label>
                                                <textarea name="data_details[]" cols="30" rows="1" class="form-control form-control-sm w-100" placeholder="Investigation result"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-2 mt-2 text-right">
                                        <button id="inv-submit" type="submit" class="btn btn-sm btn-primary">
                                            <div class="mr-2 loader spinner-border-sm spinner-border text-white d-none" role="status"><span class="sr-only">Loading...</span></div>Add New
                                        </button>
                                    </div>
                                </form>
                            </td>
                ...
                $.get(`{{ route('admin.investigations.index') }}?user=${HISTORY_USER}`, function(response){
                    ...
                    if( response.status && response.data.length > 0 ){
                        DATA = response.data.map(item=>(
                            `<tr data-toggle="collapse" data-target="#accordion-inv-${item.id}" class="clickable"><td>${item.title}</td><td>${item.details}</td><td>${item.updated_at}</td><td class="text-right"><button type="button" data-id="${item.id}" class="btn btn-danger btn-sm p-1 rounded-pill inv-remove-btn"><span class="btn-icon icofont-trash"></span></button></td></tr><!-- Separator --><tr><td colspan="4" id="data-${item.id}"><div id="accordion-inv-${item.id}" class="collapse"><table class="table table-sm table-info"><thead><tr><th>Date</th><th>Details</th></tr></thead><tbody class="inv-items">${serializeData(item.data)}</tbody>${add_form(item.update_link)}</table></div></td></tr>`
                        ));
                    }
        ...
        $('#add-investigations').ajaxForm({ 
            ...
            success: function(response, status){
                if( response.status && status === 'success' ){
                    ...
                    $('#investigation-table').prepend(`<tr data-toggle="collapse" data-target="#accordion-inv-${id}" class="clickable"><td>${title}</td><td>${details}</td><td>${updated_at}</td><td class="text-right"><button type="button" data-id="${id}" class="btn btn-danger btn-sm p-1 rounded-pill inv-remove-btn"><span class="btn-icon icofont-trash"></span></button></td></tr><!-- Separator --><tr><td id="data-${id}" colspan="4"><div id="accordion-inv-${id}" class="collapse"><table class="table table-sm table-info"><thead><tr><th>Date</th><th>Details</th></tr></thead><tbody class="inv-items">${serializeData(data)}</tbody>${add_form(update_link)}</table></div></td></tr>`);
